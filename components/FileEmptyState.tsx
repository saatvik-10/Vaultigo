...
export default function FileEmptyState({ activeTab }: FileEmptyStateProps) {
  return (
    <Card className='border border-pink bg-slate-50'>
      <CardBody className='text-center py-16'>
        <File className='h-16 w-16 mx-auto text-blue mb-6' />
        <h3 className='text-xl text-pink font-medium mb-2'>
          {activeTab === 'all' && 'No files available'}
          {activeTab === 'starred' && 'No starred files'}
          {activeTab === 'trash' && 'Trash is empty'}
        </h3>
        <p className='text-blue mt-2 max-w-md mx-auto'>
          {activeTab === 'all' &&
